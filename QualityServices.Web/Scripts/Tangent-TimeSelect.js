		...
		function () {
			...
			if (!jQuery('#ptTimeSelectCntr').length) {
				jQuery("body").append(
					...
					+	'				<span id="ptTimeSelectUserSelAmPm" style="display:none">AM</span>'
					+	'			</div>'
					...
					+	'		</div>'
					+	'		<!--[if lte IE 6.5]>'
					+	'			<iframe style="display:block; position:absolute;top: 0;left:0;z-index:10000;'
...
jQuery.ptTimeSelect.setHr = function (h) {
    if (h.toLowerCase() == "am"
        ...
        var amPM = Number(jQuery('#ptTimeSelectUserSelHr').text().valueOf());
        if (h.toLowerCase() == "am" && amPM > 12) {
            var hrVal = jQuery('#ptTimeSelectUserSelHr').text().valueOf();
            var tempHr = Number(hrVal.valueOf()) - 12;
            if (tempHr == 12)
                tempHr = 0;
            jQuery('#ptTimeSelectUserSelHr').empty().append(tempHr);
        } else if (h.toLowerCase() == "pm" && amPM <= 12) {
            var hrVal = jQuery('#ptTimeSelectUserSelHr').text().valueOf();
            var tempHr = Number(hrVal.valueOf()) + 12;
            if (tempHr == 24)
                tempHr = 0;
        ...
        var amPM = Number(jQuery('#ptTimeSelectUserSelHr').text().valueOf());
        var hrName = jQuery('#ptTimeSelectUserSelAmPm').text();

        if (hrName.toLowerCase() == 'am') {
            var hrVal = Number(h.valueOf());
            jQuery('#ptTimeSelectUserSelHr').empty().append(hrVal);
        } else {
            var hrVal = Number(h.valueOf());
            var tempHr = Number(h) + 12;
            if (tempHr == 24) {
                tempHr = 0;
            }
            jQuery('#ptTimeSelectUserSelHr').empty().append(tempHr);
